...
def PrepareCost(deviceGraphName: str, deviceCostName: str, refreshCostTable: bool) -> PhysicalDeviceCost:
	...
	try:
		...
		deviceCost = GenerateLearnedDeviceCost(deviceCostName, graph, ~refreshCostTable, 5)

		ExportCost(filepath_learned, deviceCost)
		return deviceCost
	except:
		graph = ImportGraph(deviceGraphName)
		deviceCost = GenerateInitialDeviceCost(deviceCostName, graph, ~refreshCostTable, 5)
